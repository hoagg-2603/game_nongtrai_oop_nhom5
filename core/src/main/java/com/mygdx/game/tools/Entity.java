...
public class Entity extends Actor {
    //Lớp chuyển động của nhân vật, dùng để nhận biết chuyển động của nhân vật
...
//        setCurrentPosition(this.currentPosition.x, this.currentPosition.y);
    ...
    public void setDirection(Direction direction, float delta){
        ...
        switch (currentDirection){
            case DOWN:
                currentFrame = (TextureRegion) walkDown.getKeyFrame(frameTime);
                break;
            case UP:
                currentFrame = (TextureRegion) walkUp.getKeyFrame(frameTime);
                break;
            case LEFT:
                currentFrame = (TextureRegion) walkLeft.getKeyFrame(frameTime);
                break;
            case RIGHT:
                currentFrame = (TextureRegion) walkRight.getKeyFrame(frameTime);
                break;
            default:
                break;
        }
    ...
    public void faint(){
        currentFrame= (TextureRegion) fallDown.getKeyFrame(0.5f);
    }
