...
public abstract class PlayerController implements InputProcessor {
	//Dùng để điều khiển di chuyển nhân vật, nhấp vào để ngủ, nhấp vào NPC để trò chuyện
						...
						switch (object.getName()) {
							...
							case "bed":{
								if(screen.getWorldTime().getElapsedInHours()<21&&screen.getWorldTime().getElapsedInHours()>2)
									new Dialog("",screen.getSkin(),"dialog").text("Vẫn chưa đến giờ đi ngủ！").button("Xác nhận", true).show(screen.getStage());
								else {
